...
### Moderator Commands
  ...
  * `/d# [<message>]` - Calls # drinks, optionally with a message.  # can be positive or negative. Calling a negative drink number may be   used to cancel out the drink counter to zero and remove the drinkbar at the top of the page. The drink counter is automatically reset     (and the drinkbar disappears) whenever a new video is queued.
